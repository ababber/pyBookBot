...
import os
import subprocess
from scripts.test_helpers import freq_cntr_test
from scripts.test_helpers import list_dict_test
...
def test_words_counted_correctly():
  with open("./scripts/test_helpers/word_count_test.json", "r") as f:
...
def test_reports_generated_correctly():
  for book in books:
    data_filepath = f"{Path.cwd()}/scripts/test_helpers/reports/{book}"
    ...
    subprocess.run(["bash", "./scripts/test_helpers/test_cleanup0.sh"])

def test_error_responses():
  subprocess.run(["bash", "./scripts/test_helpers/test_helper0.sh"])
  ...
  subprocess.run(["bash", "./scripts/test_helpers/test_helper1.sh"])
  assert pyBookBot.main("books_test0") == "Please add books to the \'books/\' folder for analysis and ensure they are \'.txt\' files"
  subprocess.run(["bash", "./scripts/test_helpers/test_cleanup1.sh"])
